   ...
   static class Orbwalking
    {
        ...
        private static void Obj_AI_Base_OnDoCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender.IsMe)
            {
                ...
                {
                    Utility.DelayAction(30 - ping, () => Obj_AI_Base_OnDoCast_Delayed(sender, args));
                    return;
                }
        ...
        public class Orbwalker
        {
            ...
            {
                if (ObjectManager.Player.CharData.BaseSkinName == "Vayne" && Program.qmenu.Item("FocusTwoW").GetValue<bool>())
                {
                    ...
                    )
                {
                    ...
                    {
                        var enemyGangPlank = ObjectManager.Heroes.Enemies.FirstOrDefault(e => e.ChampionName.Equals("gangplank", StringComparison.InvariantCultureIgnoreCase));
            ...
            private void GameOnOnGameUpdate(EventArgs args)
            {
                try
                {
                    ...
                    Orbwalk(
                        target, _orbwalkingPoint.To2D().IsValid() ? _orbwalkingPoint : Game.CursorPosition,
            ...
            private void DrawingOnOnDraw(EventArgs args)
            {
                ...
                if (_config.Item("AACircle2").GetValue<Circle>().Active)
                {
                    foreach (var target in
                        ObjectManager.Heroes.Enemies.FindAll(target => target.IsValidTarget(1175)))
                    {
                        Render.Circle.DrawCircle(
