...
namespace Lords_Vayne.SebbyLib
{
    ...
    public class PredictionOutput
    {
        ...
        {
            get { return _unitPosition.To2D().IsValid() ? _unitPosition.SetY() : Input.Unit.ServerPosition; }
    ...
    public static class Prediction
    {
        ...
        internal static HitChance GetHitChance(PredictionInput input)
        {
            ...
            if (hero.IsChannelingImportantSpell() || hero.IsRecalling())
            {
                return HitChance.VeryHigh;
            }
            ...
            var delay = input.Delay
                        + (Math.Abs(input.Speed - float.MaxValue) > float.Epsilon
                               ...
                               : 0);
            var moveArea = hero.MovementSpeed * delay;
            ...
            if (hero.Distance(input.From) < 250 || hero.MovementSpeed < 250 || input.From.Distance(wayPoint) < 250)
            {
                return HitChance.VeryHigh;
            }
        ...
        internal static PredictionOutput GetDashingPrediction(PredictionInput input)
        {
            ...
            if (!dashData.IsBlink)
            {
                ...
                if (dashData.Path.PathLength() > 200)
                {
                    ...
                    if (timeToPoint <=
                        input.Unit.Distance(endP) / dashData.Speed + input.RealRadius / input.Unit.MovementSpeed)
                    {
                        return new PredictionOutput
                        {
        ...
        internal static PredictionOutput GetImmobilePrediction(PredictionInput input, double remainingImmobileT)
        {
            ...
            if (timeToReachTargetPosition <= remainingImmobileT + input.RealRadius / input.Unit.MovementSpeed)
            {
                return new PredictionOutput
                {
        ...
        {
            var speed = input.Unit.MovementSpeed;
        ...
        {
            speed = (Math.Abs(speed - (-1)) < float.Epsilon) ? input.Unit.MovementSpeed : speed;
    ...
    internal static class UnitTracker
    {
        ...
        {
            if (sender.ObjectType != GameObjectType.AIHeroClient) return;
