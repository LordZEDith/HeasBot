...
namespace Lords_Vayne.SebbyLib
{
    class OKTWCommon
    {
        ...
        static OktwCommon()
        {
            ...
            Obj_AI_Base.Damage += Obj_AI_Base_OnDamage;
        ...
        {
            foreach (var hero in ObjectManager.Heroes.Enemies.FindAll(hero => hero.IsValidTarget(QWER.Range + QWER.Width, true, QWER.RangeCheckFrom) && t.NetworkId != hero.NetworkId))
            {
        ...
        {
            if ((!target.IsWindingUp && target.IsRooted && !target.CanMove) || target.MovementSpeed < 50 || target.IsStunned || target.HasBuffOfType(BuffType.Stun) || target.HasBuffOfType(BuffType.Fear) || target.HasBuffOfType(BuffType.Snare) || target.HasBuffOfType(BuffType.Knockup) || target.HasBuff("Recall") ||
        ...
        {
            foreach (var enemy in ObjectManager.Heroes.Enemies.Where(enemy => enemy.IsValid() && enemy.Distance(Player.ServerPosition) < range && (enemy.HasBuff("zhonyasringshield") || enemy.HasBuff("BardRStasis"))))
            {
        ...
        private static void Obj_AI_Base_OnDoCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            {
                if (args.Target.ObjectType == GameObjectType.AIHeroClient && !sender.IsMelee() && args.Target.Team != sender.Team)
                {
        ...
        private static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (targed != null)
            {
                if (targed.ObjectType == GameObjectType.AIHeroClient && targed.Team != sender.Team && (sender.IsMelee() || !args.SData.IsAutoAttack()))
                {
            ...
            else
            {
                ...
                if (!sender.IsEnemy || sender.IsMinion || args.SData.IsAutoAttack() || sender.ObjectType != GameObjectType.AIHeroClient)
                    return;
