...
namespace Lords_Vayne.Events
{
    class AntiGapCloser
    {
        public static void AntiGapcloser_OnEnemyGapcloser(ActiveGapcloser gapcloser)
        {
            ...
            {
                if (Program.imenu.Item("AntiAlistar").GetValue<bool>() && gapcloser.Sender.ChampionName == "Alistar" &&
                    gapcloser.SkillType == GapcloserType.Targeted)
                {
                    Program.E.Cast(gapcloser.Sender, true);

                    if (Program.gmenu.Item("Gapcloser").GetValue<bool>() &&
                        Program.gmenu.Item("AntiGapcloser" + gapcloser.Sender.ChampionName.ToLower()).GetValue<bool>())
                    {
