   ...
   internal class Lords
    {
        public static void RotE()
        {
            ...
            {
               /* if (Program.zzrot.(target.ServerPosition))
                {
        ...
        {
            foreach (var enemy in ObjectManager.Heroes.Enemies.Where(x => x.IsValidTarget(Program.E.Range) && !x.HasBuffOfType(BuffType.SpellShield) && !x.HasBuffOfType(BuffType.SpellImmunity) && canBeCondemned(x)))
        ...
        public static bool canBeCondemned(AIHeroClient unit, Vector2 pos = new Vector2())
        {
            ...
            {
                for (var i = 0; i < Program.emenu.Item("PushDistance").GetValue<Slider>().CurrentValue; i += (int)unit.BoundingRadius)
                {
        ...
        {
            int distance = 0;
            for (int i = 0; i < Program.emenu.Item("PushDistance").GetValue<Slider>().CurrentValue; i += 20)
            {
        ...
        {
            var HeroList = ObjectManager.Heroes.Enemies.Where(
                h =>
            ...
            foreach (var Hero in HeroList)
            {
                ...
                int pushDist = Program.emenu.Item("PushDistance").GetValue<Slider>().CurrentValue;
